class Admin::ProductAttributesController < Admin::AdminController
  def create
    ...
    if @product_attribute.save
      flash[:notice] = I18n.t('admin.product_attribute_saved')
    end
  ...
  def update
    ...
    if @product_attribute.save
      flash[:notice] = I18n.t('admin.product_attribute_updated')
    end
    redirect_to admin_category_path(id: params[:category_id])
  end
  ...
  def destroy
    if ProductAttribute.delete(params[:id])
      flash[:notice] = I18n.t('admin.product_attribute_deleted')
    end
    redirect_to admin_category_path(id: params[:category_id])
  end
